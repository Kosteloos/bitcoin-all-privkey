...
<pre class="keys">
<strong>Private Key</strong>                                            <strong>Address Uncompressed</strong>                               <strong>Address Compressed</strong>                 <strong>Balance</strong> 
`
...
const KeyTemplate = `<span id="%s"><a href="/warning:understand-how-this-works!/%s">+</a> <span title="%s">%s </span> <a href="https://blockchain.info/address/%s">%34s</a> <a href="https://blockchain.info/address/%s">%34s</a>%10s %10s</span>
`
...
func compute(count *big.Int) (keys [ResultsPerPage]Key, length int) {
	...
	for i = 0; i < ResultsPerPage; i++ {
		...
		s := RandStringBytes(20)
        h := sha256.New()
	    h.Write([]byte(s))
        privKey2, public2 := btcec.PrivKeyFromBytes(btcec.S256(), h.Sum(nil))
        ...
        caddr2, _ := btcutil.NewAddressPubKey(public2.SerializeCompressed(), &chaincfg.MainNetParams)
		uaddr2, _ := btcutil.NewAddressPubKey(public2.SerializeUncompressed(), &chaincfg.MainNetParams)

		// Encode addresses
		wif, _ := btcutil.NewWIF(privKey, &chaincfg.MainNetParams, false)
        wif2, _ := btcutil.NewWIF(privKey2, &chaincfg.MainNetParams, false)
		keys[i].private = wif.String()
        keys[i].private2 = wif2.String()
		keys[i].number = count.String()
		keys[i].compressed = caddr.EncodeAddress()
		keys[i].uncompressed = uaddr.EncodeAddress()
        keys[i].compressed2 = caddr2.EncodeAddress()
		keys[i].uncompressed2 = uaddr2.EncodeAddress()
	}
	return keys, i
}
...
func PageRequest(w http.ResponseWriter, r *http.Request) {
	...
	for i := 0; i < length; i++ {
		key := keys[i]
		balanceu := getbalance(key.uncompressed2)
		balancec := getbalance(key.compressed2)
		if balanceu > "0" {
		  fmt.Fprintf(w, KeyTemplate, key.private2, key.private2, key.number, key.private2, key.uncompressed2, key.uncompressed2, key.compressed2, key.compressed2, balanceu, balancec)
		} else if balancec > "0" {
		  fmt.Fprintf(w, KeyTemplate, key.private2, key.private2, key.number, key.private2, key.uncompressed2, key.uncompressed2, key.compressed2, key.compressed2, balanceu, balancec)
	    }
    }
